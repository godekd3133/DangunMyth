...
public class S1C15 extends Scene {
  ...
  @Override public void OnDraw() {
    ...
    for (Iterator<Obstacle> iterator = obstacleList.iterator(); iterator.hasNext();) {
        ...
        if (collisionTime == 0f) {
          if ((hwanungX > obstacle.x && hwanungX < (obstacle.x + obstacleImage.width * obstacle.imageScale)) 
              || ((hwanungX + hwanungImage.width * (hwanungImageScale - 0.1)) > obstacle.x && (hwanungX + hwanungImage.width * (hwanungImageScale - 0.1)) < (obstacle.x + obstacleImage.width * obstacle.imageScale))) { 
            if ((hwanungY > obstacle.y && hwanungY < (obstacle.y + obstacleImage.height * obstacle.imageScale)) 
                || ((hwanungY + hwanungImage.height * (hwanungImageScale - 0.1)) > obstacle.y && (hwanungY + hwanungImage.height * (hwanungImageScale - 0.1)) < (obstacle.y + obstacleImage.height * obstacle.imageScale))) {
    ...
    if (collisionTime != 0f) {
      ...
      switch (collisionObstacle) {
        ...
        case "bird" : case "plane" :
          hwanungX += collisionDirection == 1? collisionActionSpeed * -1 : collisionActionSpeed;
        ...
        case "gust" : 
          ...
          break;
        case "UFO" : 
          obstacleIntervalTime *= 0.9;
          altimeter += altimeterSpeed;
          break;
        case "cloud" : 
          image.DrawImageScale("fog", new PVector(width / 2, height / 2, 0), new PVector(1.2, 1.2, 0));
          break;
        case "lightning" :
          if (collisionLightning[collisionLightningIndex] == 0) {
